...
//        regexp[i]=GenerateRegExp(preparedRequest);
    ...
    {
        occurences = getWordOccurences0(regexp[j]);
...
function displayWarning(messageType,clear) {
	var warn = "";
	if (messageType == "") {
	} else if (top.warningMessages[messageType]) {
		warn = top.warningMessages[messageType];
	} else {
		warn = messageType;
	}
	var warningBlock = document.getElementById("warningBlock");
	if (clear) {
		while (warningBlock.firstChild != null) {
			warningBlock.removeChild(warningBlock.firstChild);
		}
		var childs = warningBlock.childNodes;
		for (var i;i<childs.length;i++) {
			warningBlock.removeChild(childs[i]);
		}
	}
	if (warn.length > 0) {
